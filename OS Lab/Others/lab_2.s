...
mov ax, 40
mov fs, ax                  ; gs stored in descriptor 6
mov ax, 48
mov gs, ax                  ; fs stored in descriptor 7
...
;------------------------------------------------------------------ 
; Divide by zero : Interrupt generation - 10/0
;------------------------------------------------------------------
mov eax, 10
mov edx, 0
mov ecx, 0
idiv ecx
;Verification of return from interrupt 0
mov dword[gs:4], 0x11111111
;-------------------------------------------------------------------
; End of divide by zero
;-------------------------------------------------------------------
...
;------------------------------------------------------------------
int 4
; Verification of return from interrupt 4
mov dword[gs:12], 0x22222222
;-------------------------------------------------------------------
; End of interrupt 4
;-------------------------------------------------------------------
...
;------------------------------------------------------------------ 
; The Interrupt 11 - Segment not present
;------------------------------------------------------------------
mov dword[es:28], 0x00401200 	; Manually setting the present bit to 0
mov ax, 24			; Reloading the descriptor shadow registers
mov ds, ax
mov dword[ds:1], 0xbcabcabc 	; Generating segement not present interrupt
mov dword[gs:20], 0x33333333	; Verification of return from interrupt
;-------------------------------------------------------------------
; End of interrupt 11
;-------------------------------------------------------------------
...
;------------------------------------------------------------------ 
; The Interrupt 13 - Segment Limit Exceeded
;------------------------------------------------------------------
mov eax , dword[ds:780] 	; Generating the interrupt by accessing data beyond limit
mov dword[gs:28],0x44444444	; Verification of return from interrupt
;-------------------------------------------------------------------
; End of interrupt 13
;-------------------------------------------------------------------
jmp chanHappy
...
;;;;;;;;; ISR  for Interrupt 0 ;;;;;;;;;;;;
align 0x100
mov dword[gs:0], 0x1111dead ; Verification for entry into ISR
mov ecx, 2
iretd

;;;;;;;;; ISR for Interrupt 4  ;;;;;;;;;;;;
align 0x100
mov dword[gs:8], 0x2222dead ; Verification for entry into ISR
iretd

;;;;;;;;; ISR for Interrupt 11 ;;;;;;;;;;;;
align 0x100
mov dword[gs:16], 0x3333dead 	; Verification for entry into ISR
mov dword[es:28], 0x00409200 	; changing the present bit back to 1
mov ax, 24			; Reloading the shadow registers
mov ds, ax
pop eax
iretd

;;;;;;;; ISR for Interrupt 13 ;;;;;;;;;;;;;
align 0x100
mov dword[gs:24], 0x4444dead	; Verification for entry into ISR
mov  dword[es:24] , 0xb000031f	; Changing limit of data segment back to 0x31f(init 0x300)
mov ax, 24			; Reloading the shadow registers
mov ds, ax
pop eax
iretd
...
; Data Segment Descriptor
 dd 0xb0000300, 0x00409000
; Data Segment Descriptor of GDT 
 dd 0xd0000100, 0x00409200
;Data Segment Descriptor
 dd 0xbc80095f, 0x00409000
;Data Segment Descriptor for checking outputs.
 dd 0xc5e0095f, 0x00409200
...
;Exception gate for type 0
 db 0x00 ; 0-7  bits of offset
 db 0x01 ; 8-15 bits of offset
 db 0x8  ; LSB of segment selector
 db 0x0  ; MSB of segment selector
 db 0x0  ; Reserved + 000
 db 0x8e ; Implemented as an Interrupt Gate of PL 0.
 db 0x0 ; 16-23 bits of offset
 db 0x0 ; 23-31 bits of offset
...
;Exception gate for type 4
 ...
 db 0x0
 db 0x0
 db 0x8f ; Implemented as a Trap Gate of PL 0
 db 0x0
 db 0x0
