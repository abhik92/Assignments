...
;   currently computed.
mov ebx ,  0    ;;  ebx is equivalent to i;;
mov edi , 0     ;; For writing the output of assembly into GS ;;
loop1: 
	mov ecx , 0         ;; ecx is equivalent to j ;;
	
	loop2:
		mov esi, 0      ;; esi is equivalent to k ;;
		mov dword[GS : edi]  , 0     ;; storing the lower order 32-bits of result ;;
		mov dword[GS :edi+4] , 0     ;; storing the higher order 32-bits of result ;;
		
		loop3:
		    
		    ;; Compute (20*i+k)*4 for the value of m1[i][k] ;;
		    mov eax , ebx 
		    imul eax, 20
		    add eax , esi
		    shl eax , 2
		    ...
		    ;; Compute (30*k+j)*4 for the value of m2[k][j] ;;
		    mov eax , esi
		    imul eax, 30
		    add eax , ecx
		    shl eax , 2
		    ...
		    add dword[GS :edi] , eax
		    adc dword[GS : edi+4] , edx
		...
		;KAMAC2
   	        ...
   	        add edi , 8     ;; Since the output is an 8-byte number edi is incremented by 8 ;;
		inc ecx
		cmp ecx, 30
		jne loop2
	inc ebx
	...
	jne loop1
      ...
      ;KAMAC1 jmp exit1
;;;;;;;;;;;;;;;;Comparison of C output and assembly ouput;;;;;;;;;;;;;;;;;
mov eax , 0
loop4:
	mov edx, dword[GS : eax]
	cmp dword[FS : eax]  , edx
	jne error
	add eax,4
	cmp eax , 2400
	jl loop4
