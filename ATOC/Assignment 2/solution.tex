...
\]
 ...
 $FIN$ is $\Sigma_{2}$-complete, while $REG$ is $\Sigma_{3}$-complete . Hence, every language $L$ in $\Sigma_{3}$ can be $L\leqslant_{T}REG$ . Since,
 $\Sigma_{2}\subset\Sigma_{3}$ therefore , every language $K$ in $\Sigma_{2}$ can be $K\leqslant_{T}REG$. In particular, $FIN\leq_{T}REG$.
