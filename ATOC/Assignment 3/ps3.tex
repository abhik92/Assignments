...
\begin{parts}
\part Fix a number $n$. Show that the set $$\mathcal{W}_n = \{ y \mid M_n \textrm{ accepts } y \}$$ is complete for the class of semi-decidable languages.
\part For every function $f$ that can be computed by a total Turing machine, prove that there exists an $n$ such that $\mathcal{W}_n = \mathcal{W}_{f(n)}$
\end{parts}
...
\begin{parts}
\part Given a Turing machine $M$, test if $M$ is Productive.
\part Given a Turing machine $M$, test if $M$ is Simple.
\end{parts}
