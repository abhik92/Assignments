...
Divide the set of vertices in the graph into set $A$ and $V\setminusA$. \newline
...
We can show that $\overine{BIPARTITE} \in \NL$ from the result in part(a). That is given a graph $G$, if there exists a odd cycle, then $G \in \overline{BIPARTITE}$.
