...
\begin{solution}
 \begin{parts}
  \part
    ...
    \part
    The repeated squaring technique uses the following algorithm \newline
    \begin{verbatim}
    ...
    From the above code , the space requirement for every recursive call is O($\log (n)$) for the boolean matrix multiplication. Hence, the recurrence for space can be written as: \newline
    $S(p,n)=S(p/2,n) + O(\log (n))$. \newline
    
    This gives $S(p,n)$ = O($\log (n) \log (p)$).
