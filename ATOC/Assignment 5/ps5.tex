...
If not, assign to all variables of form $x_{j}$ a true and $\overline{x_{j}}$ a false value. Else, if there is no such clause, report unsatisfiable. \newline
Hence, any $\SAT$ problem can be solved in polynomial time, since the scanning takes atmost the length of the clause. And since, $\SAT$ is $\NP$-Complete,
