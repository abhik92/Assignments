...
int main()
{
    ...
    long int height[400005];

    long int i;
    for(i=0;i<n;i++)
        height[i] = obj.ReadNext();    

    deque< pair<long int,long int> > windowMax;
    deque< pair<long int,long int> > windowMin;
    ...
    for(i=0;i<n;i++)
    {
        while (!windowMin.empty() && windowMin.back().first >= height[i])
            windowMin.pop_back();
        
        windowMin.push_back(make_pair(height[i], i));

        while(windowMin.front().second <= i - k)
            windowMin.pop_front();
        
        while (!windowMax.empty() && windowMax.back().first <= height[i])
            windowMax.pop_back();
        
        windowMax.push_back(make_pair(height[i], i));

        while(windowMax.front().second <= i - k)
            windowMax.pop_front();

        if(i>=k && (windowMax.front().first-windowMin.front().first +1) == k)
            ans++;
